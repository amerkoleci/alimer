...
namespace
{
    ...
    constexpr WindowEventType ToWindowEventType(uint32_t id)
    {
        switch (id)
        {
            ...
            case SDL_EVENT_WINDOW_MOUSE_ENTER:          return WindowEventType_MouseEnter;
            case SDL_EVENT_WINDOW_MOUSE_LEAVE:          return WindowEventType_MouseLeave;
            ...
            case SDL_EVENT_WINDOW_DISPLAY_CHANGED:      return WindowEventType_DisplayChanged;
            case SDL_EVENT_WINDOW_DISPLAY_SCALE_CHANGED: return WindowEventType_DisplayScaleChanged;
            case SDL_EVENT_WINDOW_SAFE_AREA_CHANGED:    return WindowEventType_SafeAreaChanged;
            case SDL_EVENT_WINDOW_OCCLUDED:             return WindowEventType_Occluded;
            case SDL_EVENT_WINDOW_ENTER_FULLSCREEN:     return WindowEventType_EnterFullscreen;
            case SDL_EVENT_WINDOW_LEAVE_FULLSCREEN:     return WindowEventType_LeaveFullscreen;
            default:
...
bool alimerPlatformInit(void)
{
    ...
    alimerLogInfo(LogCategory_Platform, "SDL Initialized: v%d.%d.%d, revision: %s, video driver: %s",
...
bool alimerWindowGetMousePosition(Window* window, float* x, float* y)
{
    ...
    SDL_MouseButtonFlags flags = SDL_GetGlobalMouseState(&globalX, &globalY);

    int window_x{};
    int window_y{};
    SDL_GetWindowPosition(window->handle, &window_x, &window_y);
    if (x)
        *x = globalX - window_x;
    if (y)
        *y = globalY - window_y;
...
void* alimerWindowGetNativeDisplay(Window* window)
{
    ...
    const SDL_PropertiesID props = SDL_GetWindowProperties(window->handle);

#if defined(SDL_PLATFORM_WIN32)
    return SDL_GetPointerProperty(props, SDL_PROP_WINDOW_WIN32_INSTANCE_POINTER, nullptr);
...
PowerLineStatus alimerGetPowerLineStatus(void)
{
    ...
    switch (state)
    {
        case SDL_POWERSTATE_UNKNOWN: return PowerLineStatus_Unknown;
        case SDL_POWERSTATE_ON_BATTERY: return PowerLineStatus_Offline;
        case SDL_POWERSTATE_CHARGING: return PowerLineStatus_Online;
        default: return PowerLineStatus_Unknown;
    }
}

BatteryStatus alimerGetBatteryStatus(int* batteryLifeTime, int* batteryLifePercent)
{
    SDL_PowerState state = SDL_GetPowerInfo(batteryLifeTime, batteryLifePercent);
    switch (state)
    {
        case SDL_POWERSTATE_UNKNOWN: return BatteryStatus_Unknown;
        case SDL_POWERSTATE_NO_BATTERY: return BatteryStatus_NotPresent;
        case SDL_POWERSTATE_ON_BATTERY: return BatteryStatus_OnBattery;
        case SDL_POWERSTATE_CHARGING: return BatteryStatus_Charging;
        case SDL_POWERSTATE_CHARGED: return BatteryStatus_Charged;
        default: return BatteryStatus_Unknown;
    }
}

/* TODO */
/* Add support for SDL_PowerState SDL_GetGamepadPowerInfo(SDL_Gamepad* gamepad, int* percent) */
