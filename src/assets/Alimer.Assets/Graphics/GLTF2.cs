...
public partial class Gltf2
{
    ...
    public enum ComponentType
    {
        ...
        Float = 5126,
        /// <summary>
        /// OpenGL double type. Note that some of glTF 2.0 validator does not support double type even the schema seems allow any value of integer:
    ...
    )
    {
        ...
        public readonly int ByteStride(BufferView bufferView)
        {
            if (!bufferView.ByteStride.HasValue)
            {
                ...
                int numComponents = GetCountOfAccessorType(Type);
                if (numComponents <= 0)
                {
                    return -1;
                }
            ...
            else
            {
                ...
                int bufferViewByteStride = bufferView.ByteStride.Value;
                if ((bufferViewByteStride % componentSizeInBytes) != 0)
                {
                    return -1;
                }

                return bufferViewByteStride;
            }
        }
    }
    ...
    public static int GetComponentSizeInBytes(ComponentType componentType)
    {
    ...
    private class FloatStructJsonConverter<T> : JsonConverter<T> where T : unmanaged
    {
        ...
        {
            Span<float> values = stackalloc float[s_count];
            int length = 0;

            if (reader.TokenType == JsonTokenType.StartArray)
            {
                // read values until the end
                while (reader.Read() && reader.TokenType == JsonTokenType.Number && length < s_count)
                {
        ...
        {
            ReadOnlySpan<float> values = MemoryMarshal.Cast<T, float>([value]);
