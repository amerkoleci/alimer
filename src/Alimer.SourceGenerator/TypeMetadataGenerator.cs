...
internal sealed class TypeMetadataGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        {
            GenerateModuleInit(context, types);

            foreach (TypeMetadataGenerate typeGenerate in types)
            {
                string source = Build(typeGenerate);
                context.AddSource($"{typeGenerate.Symbol.Name}.g.cs", source);
            }
        }
    }

    private static void GenerateModuleInit(SourceProductionContext context, ImmutableArray<TypeMetadataGenerate> types)
    {
        ...
        builder
            ...
            .AppendOpenBracer()
                ...
                .AppendLineNoIndent($"#pragma warning restore CA2255")
        ...
        foreach (TypeMetadataGenerate typeGenerate in types)
        {
            ...
            if (symbol.DerivesFrom("global::Alimer.Engine.Component"))
            {
                //builder.AppendLine($"global::Alimer.Engine.ComponentRegistry.RegisterComponent<{symbol.ToDisplayString()}>();");
                builder.AppendLine($"global::Alimer.MetadataRegistry.RegisterObjectType<{symbol.ToDisplayString()}>();");
            }
        }
        ...
        string moduleInitSource = builder.ToString();

        context.AddSource("ModuleInit.g.cs", SourceText.From(moduleInitSource, Encoding.UTF8));
    }
    ...
    public static string Build(in TypeMetadataGenerate typeGenerate)
    {
        ...
        builder
            ...
            .AppendLine()
            .AppendLine($"{symbol.DeclaredAccessibility.ToDisplayString()} partial class {symbol.Name}")
            .AppendOpenBracer()
