...
namespace Alimer
{
    ...
    ALIMER_ENUM_CLASS_FLAG_OPERATORS(KeyModifiers);
    ...
    ALIMER_ENUM_CLASS_FLAG_OPERATORS(MouseButtons);

    /// Maximum number of keys the input will track.
    static constexpr uint32_t kMaxKeyboardKeys = Limits<uint8_t>::Max;

    /// Maximum number of mouse buttons the input will track.
    static constexpr uint32_t kMaxMouseButtons = 8;

    /// Maximum number of controllers the input can handle
    static constexpr uint32_t kMaxControllers = 8;

    /// Maximum number of buttons the input will track
    static constexpr uint32_t kMaxControllerButtons = 64;

    /// Maximum number of controller axis the input will track
    static constexpr uint32_t kMaxControllerAxis = 16;

    /// Keyboard State
    struct KeyboardState final
    {
        /// Whether a key was pressed this frame
        bool pressed[kMaxKeyboardKeys];

        /// Whether a key is currently held
        bool down[kMaxKeyboardKeys];

        /// Whether a key was released this frame
        bool released[kMaxKeyboardKeys];
    };
    ...
    struct MouseState final
    {
        ...
        bool pressed[kMaxMouseButtons];

        // Whether a button was held this frame.
        bool down[kMaxMouseButtons];

        // Whether a button was released this frame.
        bool released[kMaxMouseButtons];
    ...
    /// An Input State, which stores the state for keyboard, mouse and gamepads
    ...
    /// Defines Input module.
    class ALIMER_API Input final
    {
    public:
        /// Event fired on key pressed.
        static Signal<KeyboardKeys, KeyModifiers> KeyDown;

        /// Event fired on key released.
        static Signal<KeyboardKeys, KeyModifiers> KeyUp;

        /// Event fired when mouse moves.
        static Signal<const Vector2&> MouseMove;

        /// State for the current frame
        static InputState currentState;

        /// State for the previous frame.
        ...
        static bool IsKeyboardKeyDown(KeyboardKeys key);
        static bool IsKeyboardKeyPressed(KeyboardKeys key);
        static bool IsKeyboardKeyReleased(KeyboardKeys key);
        ...
        /// React to a keyboard key. Called by window message handling.
        static void OnKeyboardKey(KeyboardKeys key, KeyModifiers modifiers, bool pressed);

        /// React to a mouse button. Called by window message handling.
        static void OnMouseButton(MouseButton button, float x, float y, bool pressed);

        /// React to a mouse motion. Called by window message handling.
        static void OnMouseMove(float x, float y);

        /// React to a mouse wheel. Called by window message handling.
        static void OnMouseWheel(float xOffset, float yOffset);
    ...
    private:
        ...
        ALIMER_DISABLE_COPY_MOVE(Input);
    };
}
