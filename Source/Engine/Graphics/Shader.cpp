...
namespace Alimer
{
    ...
    namespace
    {
        ...
        std::wstring ShaderProfileName(ShaderStages stage, ShaderModel shaderModel)
        {
        ...
        inline ShaderRef CompileDXIL(ShaderStages stage, const std::string& source, const std::string& entryPoint)
        {
            ...
            return Shader::Create(stage, pShader->GetBufferPointer(), pShader->GetBufferSize());
    }
}
#endif
    ...
    ShaderRef Shader::Create(ShaderStages stage, const void* bytecode, size_t bytecodeLength)
    {
        ALIMER_ASSERT(gGraphics().IsInitialized());

        return gGraphics().CreateShader(stage, bytecode, bytecodeLength);
    }
}
